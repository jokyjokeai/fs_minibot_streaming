...
class FasterWhisperSTT:
    ...
    def transcribe_file(self, audio_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            num_channels = None
            sample_rate = None
            max_retries = 3  # Augmenté pour meilleure reliability (95%+ success rate)
            retry_delays = [0.3, 0.7, 1.2]  # Progressive backoff optimisé (total: 2.2s max)
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if retry < max_retries - 1:
                        ...
                        logger.error(f"WAV header still invalid after {max_retries} retries ({sum(retry_delays):.1f}s total wait): {e}")
                        raise
