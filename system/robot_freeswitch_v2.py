"""
RobotFreeSWITCH V2 - MiniBotPanel v3
====================================

Version propre et complète du robot FreeSWITCH avec:
- Dual ESL connections (events + API)
- Streaming audio FreeSWITCH → WebSocket
- Mode classique (intent mapping simple)
- Mode agent autonome (objections, max_turns)
- AMD detection
- Background audio
- Gestion robuste des erreurs
- Database tracking complet
...
Architecture:
    ...
    5. Speech Recognition (Vosk via StreamingASR)
...
class RobotFreeSwitchV2:
    """
    Robot FreeSWITCH V2 - Version propre et complète

    Gère les appels sortants avec:
    ...
    def __init__(self):
        ...
        logger.info("="*60)
        logger.info("🤖 RobotFreeSWITCH V2 - Initialization")
        logger.info("="*60)
        ...
        # 4. Streaming ASR (WebSocket server)
        try:
            ...
            if self.streaming_asr.is_available:
                ...
                logger.info(f"✅ StreamingASR server started in background (port 8080)")
            else:
                logger.warning("⚠️ StreamingASR not available - missing dependencies")
        except Exception as e:
        ...
        logger.info("✅ RobotFreeSWITCH V2 initialized")

    def __repr__(self):
        return f"<RobotFreeSwitchV2 active_calls={len(self.active_calls)} running={self.running}>"
    ...
    def start(self):
        ...
        logger.info("Starting RobotFreeSWITCH V2...")
        ...
        logger.info("✅ RobotFreeSWITCH V2 started and listening for events")
    ...
    def stop(self):
        ...
        logger.info("Stopping RobotFreeSWITCH V2...")
        ...
        logger.info("✅ RobotFreeSWITCH V2 stopped")
        ...
        try:
            if event_type == "transcription":
                ...
                if call_uuid in self.barge_in_active and not self.barge_in_active[call_uuid]:
                    # STAGE 1: NE PAS trigger barge-in immédiatement sur speech_start
                    # On attend la transcription (STAGE 2) pour décider si backchannel ou interruption
                    # Sauf si le client parle depuis TRÈS longtemps (>3.5s)

                    # Calculer durée parole CLIENT depuis fin parole robot
                    robot_end_time = self.streaming_sessions[call_uuid].get("robot_speech_end_time", 0)

                    # Si robot n'a pas encore fini (robot_end_time = 0), c'est un vrai barge-in pendant audio
                    if robot_end_time == 0:
                        # Robot parle encore → Attendre transcription pour décider
                        logger.debug(f"[{call_uuid[:8]}] 🎤 Speech during robot audio - waiting for transcription to decide")
                        return

                    # Robot a fini → Calculer durée parole client
                    speech_duration = current_time - robot_end_time

                    # Si parole TRÈS longue (>3.5s depuis fin robot), barge-in immédiat
                    if speech_duration > config.BACKCHANNEL_MAX_DURATION:
                        logger.info(f"[{call_uuid[:8]}] 🔊 Barge-in detected (speech_start, client speech > {config.BACKCHANNEL_MAX_DURATION}s: {speech_duration:.1f}s since robot finished)")
                        self.streaming_sessions[call_uuid]["barge_in_detected_time"] = time.time()
                        return

                    # Sinon, attendre la transcription (STAGE 2) pour analyse backchannel
